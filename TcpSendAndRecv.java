...
class TT implements Runnable{
    ...
    public void run() {
        try {
            Socket s=  new Socket("localhost",9977);//创建一个socket绑定的端口和地址为：9977，本机。
...
class DD implements Runnable{
    ...
    public void run() {
        try {
            ServerSocket ss  = new ServerSocket(9977);//创建一个serversocket其端口与发送端的端口是一样的
